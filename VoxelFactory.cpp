...
void VoxelFactory::FromVoxTransform(const ogt_vox_transform& T, float3& outPos, float3& outRot, float3& outScale, float3 pivot)
{
    ...
    // approximate scale (column lengths)
    ...
    // rotation extraction (assumes no shear, uniform scale)
    ...
    float3 scale)
{
    ...
    // Save the instance in the scene first
